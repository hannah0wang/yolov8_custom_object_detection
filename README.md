...
\[insert section number\]), we realized that the dataset was rather
...
![Example training dataset photos with augmentations applied to improve
model accuracy.](roboflow-augmentations.png){#fig:augment width="7cm"}
...
![Example dataset split sourced from Statology
(Zach).](set-splits.png){#fig:split width="7.5cm"}
...
![Example frames from the dashcam video after running the detection
model.](old-model-video.png){#fig:oldvid width="9.5cm"}
...
![Left: Model correctly detecting an overpass instance. Right: Model
correctly identifying no overpasses.](new-model-video.png){#fig:newvid
width="9.5cm"}
...
![Example confusion matrix graphic with calculations for precision and
recall metrics. Graphic sourced from
Roboflow.](roboflow-conf-matrix.png){#fig:robomatrix width="8cm"}

![The resulting confusion matrix after training the model on the initial
custom overpass dataset (validation
set)](old-confusion-matrix.png){#fig:oldmatrix width="6.5cm"}
...
![The resulting confusion matrix after training the model on the
improved custom overpass dataset (validation set)
](new-confusion-matrix.png){#fig:newmatrix width="6cm"}
...
![Instances where overpasses went undetected in the improved
model.](undetected-overpasses.png){#fig:undetect width="8.5cm"}
...
![Plots of box loss, classification loss, and objectness loss for the
initial dataset.](old-loss.png){#fig:oldloss width="8.5cm"}

![Plots of box loss, classification loss, and objectness loss for the
improved dataset.](new-loss.png){#fig:newloss width="8.5cm"}
...
## Extracting Ground Truth from Video Frame Predictions
...
![A segment of the output generated during the model's object detection
process.](stream-output-small.png){#fig:stream width="9.5cm"}
...
![A segment of the outputs saved to text
file.](save-conf-true.png){#fig:saveconf width="8cm"}

![An excerpt from the combined text file provides timestamp associations
for each prediction.](combined-txt-file.png){#fig:combtxt width="8cm"}
...
%20algorithm%20to,an%20unbiased%20estimate%20of%20the\
%20final%20model%20performance.
